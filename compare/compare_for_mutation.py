...
if __name__ == '__main__':
    ...
    for pixel in range(784):
        B_model = '../mutation_model/DIN/predict/din_' + str(pixel) + '.npy'
        B_data = np.load(B_model)
        B_predict = list(map(lambda x: np.argmax(x), B_data))
        ...
        for num in num_list:
            for k in range(20):
                ...
                print('B: din_'+str(pixel)+', C: dhn_' + location + ', similarity: ' + str(percent))
    ...
    print(np.max(result))
    print(np.min(result))
    np.save('compare_data_din_dhn.npy', result)
